...
class GraphQLPreprocessor {
    ...
    private static function get_filter_snippet ($fields)
    {
        $graphql = ' (filter: "fields:'.$fields.'")';
        return $graphql;
    }
    ...
    public static function generate($query, $extract_null_values=false, $end = true, $counter = 1) {
        ...
        {
            $top_filter_snippet=self::get_filter_snippet($query['fields']);
        }
            ...
            foreach ($query['relations'] as $key => $value) {
                ...
                if (is_array($query['relations'][$key])) {
                    foreach ($query['relations'][$key] as $key2 => $value2) {
                        if ($key2 !== 'relations' && $key2 !== 'filters' && $key2 !== 'fields') {
                ...
                if (isset($query['relations'][$key]['fields']))
                {
                    $graphql.=self::get_filter_snippet ($query['relations'][$key]['fields']);
                }
